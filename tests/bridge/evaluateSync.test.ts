...
describe("evaluateSync fake mode", () => {
  ...
  it("returns scripted answers by unit name and zeros for unknown questions", () => {
    const res = evaluateSync({ filename: "a.ts", cwd: ".", model: "m", timeoutMs: 100, concurrency: 1, cacheDir: "x", maxFunctionTokens: 6000, provider: "typesafe",
  ...
  it("returns scripted errors", () => {
    ...
    const res = evaluateSync({ filename: "a.ts", cwd: ".", model: "m", timeoutMs: 100, concurrency: 1, cacheDir: "x", maxFunctionTokens: 6000, provider: "typesafe", units: [] });
