...
export type Provider = "auto" | "typesafe" | "openrouter";
...
export interface EvaluateRequest { filename: string; cwd: string; model: string; timeoutMs: number; concurrency: number; cacheDir: string; maxFunctionTokens: number; units: EvaluateUnit[]; provider: Provider }
...
export interface Settings { model: string; timeoutMs: number; maxFunctionTokens: number; concurrency: number; cacheDir: string; strict: boolean; ignoreNames: string[]; provider: Provider }
